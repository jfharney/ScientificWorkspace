	...
	if (SW != undefined) {
		...
		$(user_info_space).append(
				'<div>Uid: <span id="user_info_id">' + SW.current_user_number
						+ '</span></div>');
		$(user_info_space).append(
				'<div>Email: ' + SW.current_user_email + '</div>');
	} else {
			...
			.ajax({
				...
				success : function(data) {
					...
					if (!jQuery.isEmptyObject(user_info_obj)) {
						...
						$(user_info_space)
								.append(
										'<div>username: ' + data['username']
												+ '</div>');
						$(user_info_space).append(
								'<div>uid: <span id="user_info_id">'
										+ data['uid'] + '</span></div>');
						$(user_info_space).append(
								'<div>email: ' + data['email'] + '</div>');
						$(user_info_space).append(
								'<div>firstname: ' + data['firstname']
										+ '</div>');
						$(user_info_space).append(
								'<div>middlename: ' + data['middlename']
										+ '</div>');
						$(user_info_space).append(
								'<div style="margin-bottom:10px">lastname: '
										+ data['lastname'] + '</div>');
