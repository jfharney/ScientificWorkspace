function getFileInfo(uid) {
  ...
  var url = 'http://' + SW.hostname + ':' + SW.port + '/constellation/files/'+uid+ '?path=|';
  ...
  $.ajax({
	...
	success: function(data) {
	  ...
	  for(var i = 0; i < data.length; i++) {
		...
		console.log('   data[i][isFolder] ' + data[i]['isFolder']);    
		children.push(child);
	  }
    	  ...
    	  success: function(response) {
    		...
    		//for(var i=0;i<node.length;i++) {
    			for(var key in response.data) {
    				console.log('  sub file key: ' + key + ' value: ' + response.data[key]);
    			}
    		//}
    	  },
        url: url,
